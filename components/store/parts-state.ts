...
function getSnapshot(): PartsState {
  try {
    ...
    if (raw !== cachedRaw) {
      cachedRaw = raw
      cachedState = parsePartsState(raw)
    }
    return cachedState
  } catch {
...
export const usePartsState = () =>
  useSyncExternalStore(subscribe, getSnapshot, () => emptyPartsState)
const subscribeReady = () => () => {}
export const usePartsReady = () =>
  useSyncExternalStore(
    subscribeReady,
    () => true,
    () => false
  )
export function updatePartsState(
  change: (current: PartsState) => PartsState
): boolean {
  try {
...
export function addPart(id: string, quantity = 1) {
  return updatePartsState((state) => ({
    ...state,
    cart: cleanCart([...state.cart, { id, quantity }]),
  }))
}
export function setPartQuantity(id: string, quantity: number) {
  return updatePartsState((state) => ({
    ...state,
    cart: cleanCart(
      state.cart.map((line) => (line.id === id ? { id, quantity } : line))
    ),
  }))
}
